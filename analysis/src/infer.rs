...
pub enum TyInfo {
    ...
    Func(TyVar, TyVar),
    Data(DataId, Vec<TyVar>, Vec<EffectVar>),
...
#[derive(Clone, Debug, PartialEq)]
pub enum EffectInfo {
    Open(Vec<EffectInstVar>, Vec<EffectVar>),
    Closed(Vec<EffectInstVar>, Option<EffectVar>),
}

impl EffectInfo {
    pub fn free() -> Self { Self::Open(Vec::new(), Vec::new()) }
}
...
pub enum EffectInstInfo {
    ...
    Known(EffectDeclId, Vec<TyVar>, Vec<EffectVar>),
}
...
impl<'a> Infer<'a> {
    ...
    pub fn with_debug(mut self, debug: bool) -> Self {
        self.debug = debug;
        if debug {
            println!("Emitting debugging output...");
        }
        self
    }
    ...
    fn set_error(&mut self, ty: TyVar) {
        ...
        if self.vars[ty.0].2.is_ok() {
            ...
            match self.vars[ty.0].1.clone() {
                ...
                TyInfo::Data(_, gen_tys, _gen_effs) => gen_tys
                    .into_iter()
                    .for_each(|ty| self.set_error(ty)),
    ...
    fn is_error(&self, ty: TyVar) -> bool {
        self.vars[ty.0].2.is_err() || match self.vars[ty.0].1.clone() {
                        ...
                        let inst = match eff {
                            Ok(EffectInst::Gen(idx, _)) => match gen_eff(idx, self) {
                                Some(eff) => {
                                    ...
                                    // TODO: Not checking this is, uh, *a choice*, I guess
                                    assert!(opener.is_none(), "TODO: Two openers in effect instantiation");
                                    opener = opener.or(Some(eff));
                                    return None;
                                },
                                None => return None, // TODO: Is it sound to infer an empty effect for one that wasn't present during instantiation?
                            ...
                            Ok(EffectInst::Concrete(decl, gen_tys, gen_effs)) => {
                                let gen_tys = gen_tys
                                    .iter()
                                    .map(|ty| self.instantiate(*ty, span, gen_ty, gen_eff, self_ty, vari))
                                    .collect();
                                let gen_effs = gen_effs
                                    .iter()
                                    .map(|e| match e {
                                        Some(e) => match self.instantiate_eff(*e, span, gen_ty, gen_eff, self_ty, vari) {
                                            Ok(Some(e)) => e,
                                            Ok(None) => self.insert_effect(span, EffectInfo::Closed(Vec::new(), None)),
                                            Err(()) => self.insert_effect(span, EffectInfo::free()),
                                        },
                                        None => self.insert_effect(span, EffectInfo::Closed(Vec::new(), None)),
                                    })
                                    .collect();
                                EffectInstInfo::Known(decl, gen_tys, gen_effs)
                            },
    ...
    pub fn instantiate(
        ...
        let info = match self.ctx.tys.get(ty) {
            ...
            Ty::Data(data, gen_tys, gen_effs) => TyInfo::Data(
                data,
                gen_tys
                    .into_iter()
                ...
                gen_effs
                    ...
                    .map(|eff| {
                        ...
                        match self.instantiate_eff(eff, span, gen_ty, gen_eff, self_ty, invariant()) {
                            Ok(Some(eff)) => eff,
                            Ok(None) => self.insert_effect(span, EffectInfo::Closed(Vec::new(), None)),
                            Err(()) => self.insert_effect(span, EffectInfo::free()),
                        }
            ...
            Ty::Assoc(inner, (class_id, gen_tys, gen_effs), assoc) => {
                ...
                let gen_effs = gen_effs
                    ...
                    .map(|eff| match eff.map(|eff| self.instantiate_eff(eff, span, gen_ty, gen_eff, self_ty, invariant())) {
                        Some(Ok(Some(eff))) => eff,
                        Some(Ok(None)) | Some(Err(())) => self.insert_effect(span, EffectInfo::Closed(Vec::new(), None)),
                        None => self.insert_effect(span, EffectInfo::free()),
                    })
                    .collect();
    ...
    fn occurs_in_eff_inner(&self, x: EffectVar, y: EffectVar, seen: &mut Vec<EffectVar>) -> bool {
        if seen.contains(&y) {
            true
        } else {
            seen.push(y);
            let r = x == y || match self.info_effect(y) {
                EffectInfo::Closed(_, None) => false,
                EffectInfo::Closed(_, Some(opener)) => self.occurs_in_eff_inner(x, opener, seen),
                EffectInfo::Open(_, eff_vars) => eff_vars
                    .into_iter()
    ...
    fn occurs_in_eff(&self, x: EffectVar, y: EffectVar) -> bool {
        self.occurs_in_eff_inner(x, y, &mut Vec::new())
    }

    fn occurs_in_ty_eff_inner(&self, x: TyVar, y: EffectVar, seen_ty: &mut Vec<TyVar>, seen_eff: &mut Vec<EffectVar>) -> bool {
        if seen_eff.contains(&y) {
            false
        } else {
            seen_eff.push(y);
            let occurs = match self.info_effect(y) {
                EffectInfo::Closed(effs, opener) => effs
                    ...
                    .any(|eff| match self.follow_effect_inst(*eff) {
                        ...
                        EffectInstInfo::Known(_, gen_tys, gen_effs) => gen_tys
                            .into_iter()
                            .any(|y| x == y || self.occurs_in_inner(x, y, seen_ty)) || gen_effs
                                .into_iter()
                                .any(|y| self.occurs_in_ty_eff_inner(x, y, seen_ty, seen_eff)),
                    }) || opener.map_or(false, |opener| self.occurs_in_ty_eff_inner(x, opener, seen_ty, seen_eff)),
                EffectInfo::Open(effs, eff_vars) => effs
                    .into_iter()
                    .any(|eff| match self.follow_effect_inst(eff) {
                        EffectInstInfo::Unknown => false,
                        EffectInstInfo::Error => false,
                        ...
                        .into_iter()
                        .any(|e| self.occurs_in_ty_eff_inner(x, e, seen_ty, seen_eff)),
            };
            seen_eff.pop();
            occurs
        }
    ...
    fn occurs_in_inner(&self, x: TyVar, y: TyVar, seen: &mut Vec<TyVar>) -> bool {
        if seen.contains(&y) {
            ...
            let occurs = match self.info(y) {
                ...
                TyInfo::Data(_, ys, ys_eff) => ys
                    .into_iter()
                    .any(|y| x == y || self.occurs_in_inner(x, y, seen)) || ys_eff
                        .into_iter()
                        .any(|eff| self.occurs_in_ty_eff_inner(x, eff, seen, &mut Vec::new())),
                    ...
                    || match self.follow_class(class) {
                        ...
                        ClassInfo::Known(_, gen_tys, gen_effs) => gen_tys
                            ...
                            .any(|y| x == y || self.occurs_in_inner(x, y, seen)) || gen_effs
                                .into_iter()
                                .any(|y| self.occurs_in_ty_eff_inner(x, y, seen, &mut Vec::new())),
                    },
                // Opaque type is not checked, it's always opaque... hopefully
                TyInfo::Effect(eff, out, _opaque) => self.occurs_in_inner(x, out, seen) || self.occurs_in_ty_eff_inner(x, eff, seen, &mut Vec::new()),
            };
            seen.pop();
    ...
    fn collect_eff_insts_inner(&self, var: EffectVar, insts: &mut Vec<EffectInstVar>, seen: &mut Vec<EffectVar>) {
        if !seen.contains(&var) {
            seen.push(var);
            match self.info_effect(var) {
                EffectInfo::Open(mut effs, eff_vars) => {
                    insts.append(&mut effs);
                    for e in eff_vars {
                        self.collect_eff_insts_inner(e, insts, seen);
                    }
                },
                EffectInfo::Closed(mut effs, opener) => {
                    insts.append(&mut effs);
                    if let Some(opener) = opener {
                        self.collect_eff_insts_inner(opener, insts, seen);
                    }
                },
            }
            seen.pop();
    ...
    fn collect_eff_insts(&self, var: EffectVar) -> Vec<EffectInstVar> {
        ...
        self.collect_eff_insts_inner(var, &mut insts, &mut Vec::new());
        insts
    }
    ...
    pub fn make_flow(&mut self, x: TyVar, y: TyVar, info: impl Into<EqInfo>) {
        ...
        if let Err((a, b)) = Self::flow_inner(&mut MakeFlow(self, &eq_info), x, y, (x, y)) {
    ...
    pub fn make_flow_effect(&mut self, x: (EffectVar, TyVar), y: (EffectVar, TyVar), info: impl Into<EqInfo>) {
        let mut eq_info = info.into();
        if let Err((a, b)) = Self::flow_effect(&mut MakeFlow(self, &eq_info), x, y, Some((x.1, y.1))) {
            if !self.is_error(a) && !self.is_error(b) {
                self.set_error(a);
                self.set_error(b);
    ...
    pub fn check_flow(&self, x: TyVar, y: TyVar) -> Option<bool> {
        ...
        let res = Self::flow_inner(&mut check, x, y, (x, y));
        match check.1 {
            Some(true) => Some(res.is_ok()),
            None => if res.is_ok() { None } else { Some(false) },
    ...
    fn flow_many<'b, I: FlowInfer<'b>>(
        ...
        xs
            .into_iter()
            .zip(ys.into_iter())
            .fold(None, |err, (x, y)| err.or(Infer::flow_inner(infer, x, y, old).err()))
    ...
    fn flow_many_effect<'b, I: FlowInfer<'b>>(
        ...
        old: Option<(TyVar, TyVar)>,
    ) -> Result<(), (TyVar, TyVar)> {
        xs
            .into_iter()
            .zip(ys.into_iter())
            .fold(None, |err, (x, y)| err.or(Infer::flow_effect(infer, (x, x_ty), (y, y_ty), old).err()))
            .map(Err).unwrap_or(Ok(()))
    }
    ...
    fn flow_effect_inst<'b, I: FlowInfer<'b>>(
        ...
        match (infer.as_ref().info_effect_inst(x), infer.as_ref().info_effect_inst(y)) {
            (EffectInstInfo::Ref(x), _) => Self::flow_effect_inst(infer, (x, x_ty), (y, y_ty), old),
            (_, EffectInstInfo::Ref(y)) => Self::flow_effect_inst(infer, (x, x_ty), (y, y_ty), old),
            ...
            (EffectInstInfo::Known(x, x_gen_tys, x_gen_effs), EffectInstInfo::Known(y, y_gen_tys, y_gen_effs)) if x == y => {
                x_gen_tys
                    .into_iter()
                    .zip(y_gen_tys)
                    .fold(None, |err, (x, y)| err.or(Infer::flow_inner(infer, x, y, old.unwrap_or((x, y))).err()))
                    .map(Err)
                    .unwrap_or(Ok(()))
                    .and_then(|_| x_gen_effs
                        .into_iter()
                        .zip(y_gen_effs)
                        .fold(None, |err, (x, y)| err.or(Infer::flow_effect(infer, (x, x_ty), (y, y_ty), old).err()))
                        .map(Err)
    ...
    fn check_eff_empty_inner(
        &self,
        (x, x_ty): (EffectVar, TyVar),
        seen: &mut Vec<EffectVar>,
    ) -> Option<bool> {
        if seen.contains(&x) {
            Some(true)
        } else {
            seen.push(x);
            let r = match self.info_effect(x) {
                EffectInfo::Open(effs, eff_vars) => Some(effs.is_empty() && eff_vars.into_iter().all(|e| self.check_eff_empty_inner((e, x_ty), seen).unwrap())),
                EffectInfo::Closed(effs, None) => Some(effs.is_empty()),
                EffectInfo::Closed(effs, Some(opener)) => Some(effs.is_empty() && self.check_eff_empty_inner((opener, x_ty), seen)?),
            };
            seen.pop();
            r
        }
    ...
    fn check_eff_empty(
        ...
        self.check_eff_empty_inner((x, x_ty), &mut Vec::new())
    }
    ...
    fn flow_effect<'b, I: FlowInfer<'b>>(
        ...
        match (infer.as_ref().info_effect(x), infer.as_ref().info_effect(y)) {
            // Checks are easy: we know nothing will change now so we can assume that any sets are exhaustive.
            (x_eff, y_eff) if infer.is_check() => {
                let y_effs = infer.infer().collect_eff_insts(y);

                if infer.infer().collect_eff_insts(x)
                    .iter()
                    .all(|x| y_effs
                        .iter()
                        .any(|y| Self::flow_effect_inst(infer, (*x, x_ty), (*y, y_ty), old).is_ok()))
                {
                    Ok(())
                } else {
                    Err((x_ty, y_ty))
            ...
            (EffectInfo::Closed(x_effs, Some(x_opener)), _) if x_effs.is_empty() => {
                Self::flow_effect(infer, (x_opener, x_ty), (y, y_ty), old)
            },
            // Closed into closed with opener
            (_, EffectInfo::Closed(y_effs, Some(y_opener))) if y_effs.is_empty() => {
                Self::flow_effect(infer, (x, x_ty), (y_opener, y_ty), old)
            },
            // When there's only one target effect, we know that all of the inputs *must* flow into it.
            (_, EffectInfo::Closed(y_effs, None)) if y_effs.len() == 1 => {
                for x in infer.infer().collect_eff_insts(x) {
                    Self::flow_effect_inst(infer, (x, x_ty), (y_effs[0], y_ty), old)?;
                }
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
            (EffectInfo::Closed(_, Some(_)), EffectInfo::Closed(_, None)) => {
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
            (EffectInfo::Closed(_, Some(_)), EffectInfo::Closed(_, Some(_))) => {
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
            (EffectInfo::Closed(x_effs, None), EffectInfo::Closed(y_effs, Some(_))) => {
                if !x_effs.iter().all(|x| y_effs.contains(x)) {
                    infer.make_check_eff((x, x_ty), (y, y_ty));
                }
                Ok(())
            },
            (EffectInfo::Open(mut x_effs, x_eff_vars), EffectInfo::Open(mut y_effs, mut y_eff_vars)) => {
                // y_effs.append(&mut x_effs);
                // infer.set_effect_info(y, EffectInfo::Open(y_effs, y_eff_vars));
                // infer.set_effect_info(x, EffectInfo::Ref(y));

                y_effs.append(&mut x_effs);
                if !infer.infer().occurs_in_eff(x, y) {
                    y_eff_vars.push(x);
                }
                infer.set_effect_info(y, EffectInfo::Open(y_effs, y_eff_vars));
                Ok(())
            ...
            (EffectInfo::Open(_, _), EffectInfo::Closed(_, Some(_))) => {
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
            (EffectInfo::Closed(_, Some(_)), EffectInfo::Open(_, _)) => {
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
            (EffectInfo::Closed(mut x_effs, None), EffectInfo::Open(mut y_effs,  y_eff_vars)) => {
                let y_eff_insts = infer.infer().collect_eff_insts(y);
                if x_effs.iter().all(|x| y_eff_insts.contains(x)) {
                    Ok(())
                } else {
                    // TODO: Instead of falling back to adding all, filter instead
                    y_effs.append(&mut x_effs);
                    infer.set_effect_info(y, EffectInfo::Open(y_effs, y_eff_vars));
                    Ok(())
                }
            },
            (EffectInfo::Open(_, _), EffectInfo::Closed(mut y_effs, None)) => {
                infer.make_check_eff((x, x_ty), (y, y_ty));
                Ok(())
            },
        }
    ...
    fn flow_class<'b, I: FlowInfer<'b>>(
        ...
        match (infer.as_ref().follow_class(x), infer.as_ref().follow_class(y)) {
            ...
            (ClassInfo::Known(class_id_x, xs, e_xs), ClassInfo::Known(class_id_y, ys, e_ys)) if class_id_x == class_id_y => {
                ...
                let co_error = Self::flow_many(infer, xs.iter().copied(), ys.iter().copied(), (x_ty, y_ty)).err();
                let contra_error = Self::flow_many(infer, ys, xs, (y_ty, x_ty)).err().map(|(a, b)| (b, a));

                // Effects are also invariant
                let e_co_error = Self::flow_many_effect(infer, (e_xs.iter().copied(), x_ty), (e_ys.iter().copied(), y_ty), Some((x_ty, y_ty))).err();
                let e_contra_error = Self::flow_many_effect(infer, (e_ys, y_ty), (e_xs, x_ty), Some((y_ty, x_ty))).err();
                co_error
                    .or(contra_error)
                    .or(e_co_error)
                    .or(e_contra_error)
    ...
    fn flow_inner<I: FlowInfer<'a>>(infer: &mut I, x: TyVar, y: TyVar, old: (TyVar, TyVar)) -> Result<(), (TyVar, TyVar)> {
        if x == y { return Ok(()) } // If the vars are equal, we have no need to check flow
        match (infer.as_ref().info(x), infer.as_ref().info(y)) {
            // Follow references
            (TyInfo::Ref(x), _) => Self::flow_inner(infer, x, y, old),
            (_, TyInfo::Ref(y)) => Self::flow_inner(infer, x, y, old),
            ...
            (TyInfo::List(x), TyInfo::List(y)) => Self::flow_inner(infer, x, y, old),
            (TyInfo::Record(xs, _), TyInfo::Record(ys, _)) if xs.len() == ys.len() && xs
                .keys()
                .all(|x| ys.contains_key(x)) => xs
                    .into_iter()
                    .try_for_each(|(x, x_ty)| Self::flow_inner(infer, x_ty, ys[&x], old)),
            (TyInfo::Func(x_i, x_o), TyInfo::Func(y_i, y_o)) => {
                let i_err = Self::flow_inner(infer, y_i, x_i, old).err().map(|(a, b)| (b, a)); // Input is contravariant
                let o_err = Self::flow_inner(infer, x_o, y_o, old).err();
            ...
            (TyInfo::Assoc(x, class_x, assoc_x), TyInfo::Assoc(y, class_y, assoc_y))
                if assoc_x == assoc_y => {
                    ...
                    let co_error = Self::flow_inner(infer, x, y, old).err();
                    let contra_error = Self::flow_inner(infer, y, x, old).err().map(|(a, b)| (b, a));
            ...
            (TyInfo::Effect(x_eff, x_out, x_opaque), TyInfo::Effect(y_eff, y_out, y_opaque)) => {
                let eff_err = Self::flow_effect(infer, (x_eff, x), (y_eff, y), Some(old)).err();
                let o_err = Self::flow_inner(infer, x_out, y_out, old).err();
                let opaque_err = Self::flow_inner(infer, x_opaque, y_opaque, old).err().map(|_| (x, y));
            ...
            EffectInfo::Open(effs, eff_vars) => {
                let effs = effs
                    ...
                    .map(|eff| match self.follow_effect_inst(*eff) {
                        ...
                        EffectInstInfo::Known(eff, gen_tys, gen_effs) => {
                            let eff_info = EffectInstInfo::Known(
                                eff,
                                gen_tys
                                    .iter()
                                ...
                                gen_effs
                                    .iter()
                                    .map(|e| self.reinstantiate_eff(span, *e))
                                    .collect(),
                            );
                            self.insert_effect_inst(span, eff_info)
                        },
                    })
                    .collect();
                let eff_vars = eff_vars
                    .into_iter()
                    .map(|e| self.reinstantiate_eff(span, e))
                    .collect();
                self.insert_effect(span, EffectInfo::Open(effs, eff_vars))
            },
    ...
    pub fn reinstantiate(&mut self, span: Span, ty: TyVar) -> TyVar {
        match self.info(ty) {
            ...
            TyInfo::Data(data, gen_tys, gen_effs) => {
                let gen_tys = gen_tys
                    .into_iter()
                    .map(|ty| self.reinstantiate(span, ty))
                    .collect();
                let gen_effs = gen_effs
                    ...
                    .collect();
                self.insert(self.span(ty), TyInfo::Data(data, gen_tys, gen_effs))
            },
                    ...
                    if let Ty::Record(fields, _) = self.ctx.tys.get(*ty) {
                        if let Some(field_ty) = fields.get(&field_name) {
                            let field_ty = self.instantiate(
                                *field_ty,
                                self.span(record),
                                &mut |index, _, _| gen_tys.get(index).copied(),
                                &mut |index, _| gen_effs.get(index).copied(),
                                Some(record),
                                invariant(),
                            );
                            if flow_out {
    ...
    fn resolve(&mut self, c: Constraint) -> Option<Result<(), InferError>> {
        match c {
            ...
            Constraint::EffectSendRecv(eff, send, recv, span) => match self.follow_effect_inst(eff) {
                ...
                EffectInstInfo::Known(decl, gen_tys, gen_effs) => {
                    let send_ty = self.instantiate(
                        ...
                        span,
                        &mut |idx, _gen_scope, _ctx| gen_tys.get(idx).copied(),
                        &mut |idx, _| gen_effs.get(idx).copied(),
                        None,
                        invariant(),
                    );
                    // Invariance. TODO: We should differentiate between handlers and propagation (see below).
                    self.make_flow(send, send_ty, span);
                    self.make_flow(send_ty, send, span);
            ...
            Constraint::EffectPropagate(obj, basin_eff, out_ty, obj_span, op_span, out_span) => match self.follow_info(obj) {
                TyInfo::Error(_) => Some(Ok(())), // Errors always succeed
                TyInfo::Effect(eff, out, _) => {
                    self.make_flow(out, out_ty, EqInfo::from(out_span));

                    // Don't allow propagation on values without an effect
    ...
    fn var_covers_var(&self, var: TyVar, ty: TyVar) -> Option<bool> {
        match (self.follow_info(var), self.follow_info(ty)) {
            ...
            (TyInfo::Data(x, xs, xs_eff), TyInfo::Data(y, ys, ys_eff)) if x == y
                && xs.len() == ys.len()
                && xs_eff.len() == ys_eff.len()
            => Some(xs
                .into_iter()
                .zip(ys.into_iter())
                .fold(Some(true), |a, (x, y)| Some(a? && self.var_covers_var(x, y)?))? && xs_eff
                    .into_iter()
                    .zip(ys_eff.into_iter())
        ...
        match (self.follow_class(x), self.follow_class(y)) {
            ...
            (ClassInfo::Known(class_id_x, x_gen_tys, x_gen_effs), ClassInfo::Known(class_id_y, y_gen_tys, y_gen_effs)) if class_id_x == class_id_y => Some(x_gen_tys
                ...
                .fold(Some(true), |a, (x, y)| Some(a? && self.var_covers_var(x, y)?))? && x_gen_effs
                    ...
                    .fold(Some(true), |a, (x, y)| Some(a? && true/*self.eff_covers_eff(x, y)?*/))?),
            (x, y) => {
                dbg!("{:?} covers {:?}", x, y);
                Some(false)
            },
    ...
    fn covers_var(
        ...
        match (self.follow_info(var), self.ctx.tys.get(ty)) {
            ...
            (_, Ty::Gen(idx, _)) => {
                ...
                // TODO: Only covariance is checked right now. This is unsound! We should derive variance for generic
                // parameters instead.
                match (self.check_flow(var, other_gen_var), self.check_flow(other_gen_var, var)) {
                    (Some(true), Some(true)) => Ok(true),
                    (Some(false), _) | (_, Some(false)) => {
                        if self.debug {
                            println!("Generic type covers, but invariance check failed between {:?} and {:?}!", self.follow_info(var), self.follow_info(other_gen_var));
                        }
                        Err(())
                    },
                    (_, _) => Ok(false),
            ...
            (TyInfo::Data(x, xs, xs_eff), Ty::Data(y, ys, ys_eff)) if x == y
                && xs.len() == ys.len()
                && xs_eff.len() == ys_eff.len()
            => Ok(xs
                .into_iter()
                .zip(ys.into_iter())
                .try_fold(true, |a, (x, y)| Ok(a && self.covers_var(x, y, ty_gens, eff_gens, todo)?))? && xs_eff
                    .into_iter()
                    .zip(ys_eff.into_iter())
                    .try_fold(true, |a, (x, y)| Ok(a /*&& self.covers_var_eff((x, var), y, ty_gens, eff_gens, todo)?*/))?),
            (TyInfo::Assoc(x, class_x, assoc_x), Ty::Assoc(y, (class_id_y, y_gen_tys, y_gen_effs), assoc_y))
                if assoc_x == assoc_y => Ok(self.covers_var(x, y, ty_gens, eff_gens, todo)? && match self.follow_class(class_x) {
                    ...
                    ClassInfo::Known(class_id_x, x_gen_tys, x_gen_effs) => class_id_x == class_id_y && x_gen_tys
                        .into_iter()
                        .zip(y_gen_tys.into_iter())
                        .try_fold(true, |a, (x, y)| Ok(a && self.covers_var(x, y, ty_gens, eff_gens, todo)?))? && x_gen_effs
                            .into_iter()
                            .zip(y_gen_effs.into_iter())
                            .try_fold(true, |a, (x, y)| Ok(a && if let Some(y) = y {
                                true/*self.covers_var_eff((x, var), y, ty_gens, eff_gens, todo)?*/
                            } else {
                                todo.push(Check::EffectEmpty((x, var), var, EqInfo::default()));
                                true
                            }))?,
                }),
            ...
            (_, Ty::Effect(y_eff, y_out)) => {
                if let Effect::Known(effs) = self.ctx.tys.get_effect(y_eff) {
                    for eff in effs {
                        match eff {
                            Ok(EffectInst::Concrete(_, _, _)) => return Ok(false),
                            Ok(EffectInst::Gen(idx, _)) => {
                                if let Some(Some((eff, var))) = eff_gens.insert(idx, None) {
                                    if self.check_eff_empty((eff, var)) != Some(true) {
                ...
                self.covers_var(var, y_out, ty_gens, eff_gens, todo)
            },
            (TyInfo::Effect(x_eff, out, _), _) => {
                if self.check_eff_empty((x_eff, var)) != Some(true) {
                    todo.push(Check::EffectEmpty((x_eff, var), var, EqInfo::default()));
                }
    ...
    fn derive_links(
        ...
        match (self.ctx.tys.get(member), self.follow_info(ty)) {
            ...
            (Ty::Data(_, xs, xs_eff), TyInfo::Data(_, ys, ys_eff)) => {
                xs
                    .into_iter()
                    .zip(ys.into_iter())
                ...
                xs_eff
                    .into_iter()
                    .zip(ys_eff.into_iter())
                    .for_each(|(x, y)| self.derive_links_eff(x, y, self_ty, use_span, ty_links, eff_links));
            },
                    ...
                    match self.follow_class(class_y) {
                        ...
                        ClassInfo::Known(class_id_y, y_gen_tys, y_gen_effs) => {
                            ...
                            x_gen_effs
                                ...
                                .zip(y_gen_effs.into_iter())
                                .for_each(|(x, y)| {
                                    let e = if let Some(x) = x {
                                        self.derive_links_eff(x, y, self_ty, use_span, ty_links, eff_links)
                                    } else {
                                        let empty = self.insert_effect(use_span, EffectInfo::Closed(Vec::new(), None));
                                        self.make_flow_effect((y, ty), (empty, ty), EqInfo::default());
                                    };
                                });
                        },
                    }
                },
            (Ty::Effect(eff, out), TyInfo::Effect(eff_var, out_var, _)) => {
                self.derive_links_eff(eff, eff_var, self_ty, use_span, ty_links, eff_links);
                self.derive_links(out, out_var, self_ty, use_span, ty_links, eff_links);
            },
    ...
    fn derive_links_eff(
        ...
        match (self.ctx.tys.get_effect(member), self.info_effect(eff)) {
            ...
            (Effect::Known(mut effs), EffectInfo::Open(_, _)) if effs.len() == 1 => {
                match &effs[0] {
            ...
            (Effect::Known(mut effs), info) if effs.len() == 1 => {
                ...
                for eff_var in self.collect_eff_insts(eff) {
                    match (&eff_id, self.follow_effect_inst(eff_var)) {
                        (Ok(EffectInst::Gen(idx, _)), _) => { eff_links.insert(*idx, eff); },
                        (Ok(EffectInst::Concrete(x, x_gen_tys, x_gen_effs)), EffectInstInfo::Known(y, y_gen_tys, y_gen_effs)) if *x == y => {
                            x_gen_tys
                                .into_iter()
                                .zip(y_gen_tys.into_iter())
                                .for_each(|(x, y)| self.derive_links(*x, y, self_ty, use_span, ty_links, eff_links));
            ...
            (x, y) => todo!("Derive links between {:?} and {:?} (effs_of(y) = {:?})", x, y, self.collect_eff_insts(eff)),
        }
    }
    ...
    fn resolve_obligation(
        &mut self,
        proof_stack: &mut Vec<(TyVar, ClassId, Vec<TyVar>, Vec<EffectVar>)>,
        ty: TyVar,
                ...
                if let Some(member) = {
                    ...
                    for member in self.implied_members.iter() {
                        if class_id == *member.class
                            ...
                            //     .fold(Some(true), |a, (eff, arg)| Some(a? && /*Self::eff_covers_eff(self, *eff, *arg)?*/true))?
                        {
                    ...
                    for (member_id, member) in self.ctx.classes.members_of(class_id) {
                        ...
                        if covers && todo.is_empty() { covering_members.push((member_id, member, todo.clone())); }
                        if maybe_covers && todo.is_empty() { maybe_covering_members.push((member_id, member, todo)); }
                    }
                    ...
                    match (covering_members.len(), maybe_covering_members.len()) {
                        ...
                        (1, _) | (0, 1) => {
                            ...
                            for member in covering_gen_scope.implied_members.as_ref().expect("Implied members must be known here").clone() {
                                // println!(
                                //     "Deriving links from {} in {:?} of class {} in {}",
                                //     self.ctx().tys.display(self.ctx(), covering_member_member),
                                //     self.follow_info(ty),
                                //     *self.ctx.classes.get(class_id).name,
                                ...
                                let proof_key = (member_ty, *member.class, member_gen_tys.clone(), member_gen_effs.clone());
                                if proof_stack.contains(&proof_key) {
                                    ...
                                    //     member_gen_effs.iter().map(|eff| format!(" {:?}", self.info_effect(*eff))).collect::<String>(),
                                    // );
    ...
    pub fn into_checked(mut self) -> (Checked<'a>, Vec<Error>) {
        ...
        let errors = errors
            ...
            .map(|error| match error {
                ...
                InferError::NotEffectful(ty, obj_span, span) => Error::NotEffectful(checked.reify(ty), obj_span, span),
            })
            .collect();
...
impl<'a> Checked<'a> {
        ...
        if let Some(ty) = self.ty_cache.get(&var) {
            *ty
            ...
            let ty = match self.infer.info(var) {
                ...
                TyInfo::Data(data, gen_tys, gen_effs) => Ty::Data(
                    data,
                    gen_tys
                        .into_iter()
                    ...
                    gen_effs
                        .into_iter()
                        .map(|eff| match self.reify_effect(eff) {
                            Some(eff) => eff,
                            None => self.infer.ctx.tys.insert_effect(self.infer.effect_span(eff), Effect::Known(Vec::new())),
                        })
                        .collect(),
                ),
    ...
    pub fn reify_effect_inst(&mut self, var: EffectInstVar) -> Result<EffectInst, ()> {
        match self.infer.follow_effect_inst(var) {
            ...
            EffectInstInfo::Known(eff, gen_tys, gen_effs) => Ok(EffectInst::Concrete(
                eff,
                gen_tys
                    .into_iter()
                    .map(|ty| self.reify_inner(ty))
            ...
            *eff
            ...
            let sort_and_order = |this: &mut Self, effs: &mut Vec<_>| {
                ...
                effs.sort_by(|x, y| match (x, y) {
                    (Ok(EffectInst::Concrete(x, x_gen_tys, x_gen_effs)), Ok(EffectInst::Concrete(y, y_gen_tys, y_gen_effs))) => x.cmp(y)
                        .then_with(|| x_gen_tys
                            .iter()
                            .zip(y_gen_tys)
                        ...
                        .then_with(|| x_gen_effs
                            ...
                            .fold(Ordering::Equal, |a, (x, y)| a.then_with(|| match (x, y) {
                                (Some(x), Some(y)) => this.infer.ctx().tys.cmp_eff(*x, *y),
                ...
                effs.dedup_by(|x, y| match (x, y) {
                    (Ok(EffectInst::Concrete(x, x_gen_tys, x_gen_effs)), Ok(EffectInst::Concrete(y, y_gen_tys, y_gen_effs))) => x == y && x_gen_tys
                        .iter()
                        .zip(y_gen_tys)
                        .all(|(x, y)| this.infer.ctx().tys.cmp_ty(*x, *y).is_eq()) && x_gen_effs
                            .iter()
                            .zip(y_gen_effs)
                            .all(|(x, y)| match (x, y) {
                                (Some(x), Some(y)) => this.infer.ctx().tys.cmp_eff(*x, *y),
